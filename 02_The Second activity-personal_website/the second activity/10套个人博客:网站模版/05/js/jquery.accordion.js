...
(function($) {
	...
	$.fn.accordion = function(config) {
		var defaults = {
			"handle" : "h5",
			"panel" : ".shortcode_accordeon_item_body",
			"speed" : 200,
			"easing" : "swing",
		...
		this.each(function() {
			var accordion = $(this), reset = {
				...
				return el;
			}).hide(), handles = accordion.find(" > li > " + defaults.handle).wrapInner('<a class="accordion-opener" href="#open-panel" />');
			...
			accordion.delegate(".accordion-opener", "click", function(e) {
				...
				var clicked = $(this), clickedLi = clicked.closest("li"), panel = clickedLi.find(">" + defaults.panel).first(), open = accordion.find(" > li:not(." + defaults.lockedClass + ") > " + defaults.panel + "." + defaults.activeClassPanel);
