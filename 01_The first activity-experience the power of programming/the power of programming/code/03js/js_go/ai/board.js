var board = function(scorePoint, R, S) {
	var count = 0
	var total = 0
	...
	class Board {
		...
		init(sizeOrBoard) {
			...
			if (sizeOrBoard.length) {
				...
				for (var i = 0; i < this.board.length; i++) {
					this.count += this.board[i].filter(function(d) {
						return d > 0;
					}).length;
				}
			} else {
			...
			this.scoreCache = [
				[], // placeholder
				[ // for role 1
					array.create(size, size), //横向得分
					array.create(size, size), //纵向得分
					array.create(size, size), //斜向得分
					array.create(size, size) //斜向得分
				],
				[ // for role 2
		...
		gen(role, onlyThrees, starSpread) {
			...
			if (starSpread && config.star) {
				...
				if (!attackPoints.length) attackPoints.push(this.currentSteps[0].role === role ? this
					.currentSteps[0] :
					this.currentSteps[1])
				if (!defendPoints.length) defendPoints.push(this.currentSteps[0].role === reverseRole ? this
					.currentSteps[0] : this.currentSteps[1])
			}
				...
				for (var j = 0; j < board.length; j++) {
					if (board[i][j] == R.empty) {
						...
						if (starSpread && config.star) {
							...
							if (maxScore >= S.FOUR) {} else if (maxScore >= S.BLOCKED_FOUR && starTo(this
									.currentSteps[
										this.currentSteps.length - 1])) {
			...
			var blockedfours = role === R.com ? comblockedfours.concat(humblockedfours) : humblockedfours
				.concat(
					comblockedfours)
