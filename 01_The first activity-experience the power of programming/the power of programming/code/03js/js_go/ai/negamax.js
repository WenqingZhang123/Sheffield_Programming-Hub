...
var negamax = function(R, T) {
	...
	var r = function(deep, alpha, beta, role, step, steps, spread) {
		...
		if (config.cache) {
			...
			if (c) {
				if (c.deep >= deep) { // 如果缓存中的结果搜索深度不比当前小，则结果完全可用
					...
					if (math.greatOrEqualThan(c.score, SCORE.FOUR) || math.littleOrEqualThan(c.score, -SCORE
							.FOUR)) {
						cacheGet++
						return c.score
					}
		...
		for (var i = 0; i < points.length; i++) {
			...
			if (_spread < config.spreadLimit) {
				// 冲四延伸
				if ((role == R.com && p.scoreHum >= SCORE.FIVE) || (role == R.hum && p.scoreCom >= SCORE
						.FIVE)) {
					// _deep = deep+1
					_deep += 2
					_spread++
	...
	var deeping = function(candidates, role, deep) {
		...
		config.log && console.log("选择节点：" + candidates[0] + ", 分数:" + result.score.toFixed(3) + ", 步数:" + result
			.step +
			', 最小值：' + result.min)
		var time = (new Date() - start) / 1000
		config.log && console.log('搜索节点数:' + count + ',AB剪枝次数:' + ABcut + ', PV剪枝次数:' + PVcut)
		config.log && console.log('搜索缓存:' + '总数 ' + cacheCount + ', 命中率 ' + (cacheGet / cacheCount * 100)
			.toFixed(3) +
			'%, ' + cacheGet + '/' + cacheCount)
		//注意，减掉的节点数实际远远不止 ABcut 个，因为减掉的节点的子节点都没算进去。实际 4W个节点的时候，剪掉了大概 16W个节点
		config.log && console.log('当前统计：' + count + '个节点, 耗时:' + time.toFixed(2) + 's, NPS:' + Math.floor(
			count /
			time) + 'N/S')
		board.log()
